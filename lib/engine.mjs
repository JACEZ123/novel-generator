...
export class PipelineRunner {
  ...
  async initBook(book) {
    ...
    const frameResp = await this.chat("architect", [
      { role: "system", content: this.skill(isScript ? "剧本结构生成" : "故事框架生成") },
    ...
    const volResp = await this.chat("architect", [
      { role: "system", content: this.skill(isScript ? "幕场结构生成" : "分卷卷纲生成") },
      { role: "user", content: `${brief}\n\n【故事框架】\n${story_frame}${customBlock}` },
...
（角色区仍用 ===ROLE=== 格式）`,
  ...
  async writeDraft(bookId, context) {
    ...
    const resp = await this.chat("writer", [
      { role: "system", content: this.skill(isScript ? "分场剧本写手" : "正文写手", { wordCount: book.chapterWordCount || 3000 }) },
      {
        role: "user",
        content: `${this.bookBrief(book)}
类型：${isScript ? "剧本" : "长篇小说"}
...
请按格式输出第 ${n} ${unit}。`,
  ...
  async auditDraft(bookId, chapterNumber) {
    ...
    const resp = await this.chat("auditor", [
      { role: "system", content: this.skill(isScript ? "剧本审计" : "章节审计") },
      {
        role: "user",
        content: `${this.bookBrief(book)}
...
请输出审计 JSON。`,
  ...
  async reviseDraft(bookId, chapterNumber) {
    ...
    const resp = await this.chat("reviser", [
      { role: "system", content: this.skill(isScript ? "剧本修订" : "章节修订") },
      {
        role: "user",
        content: `${this.bookBrief(book)}
