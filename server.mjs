...
// Node http + SSE 流式服务。写作 / 审计 / 状态引擎由写作引擎依赖提供，
// 本服务负责：编排原子操作、章纲与人物面板逻辑、前端交互与实时进度反馈。
//
// 许可：本仓库代码采用 MIT；运行时依赖的写作引擎 @actalk/inkos-core 为 AGPL-3.0，
// 你在自行部署对外服务时需遵守该依赖的开源义务。
...
// 写作引擎（外部依赖，非本仓库代码）——通过包名解析，npm install 后即可用
import { PipelineRunner, createLLMClient, StateManager, chatCompletion } from "@actalk/inkos-core";
...
function makeClient() {
  ...
  return createLLMClient({
    ...
    service: "deepseek",          // 仅作为内部服务标签；实际端点由 baseUrl 决定
    configSource: "env",
    baseUrl: cfg.baseUrl,
    model: cfg.fastModel,
    apiFormat: "chat",
...
function makeRunner({ externalContext, onDelta, onStage, reviewMode, onRequest } = {}) {
  return new PipelineRunner({
    ...
    modelOverrides: loadModelConfig(), // 分阶段模型（字符串=换模型复用同一 DeepSeek 客户端）
    externalContext,
    chapterReviewMode: reviewMode ?? "manual", // 逐步交互：默认手动分步，由前端显式触发审计
    logger: makeLogger(onStage),
    onTextDelta: onDelta,
    onRequest, // 纯观测：把实际发给 DeepSeek 的 messages 冒出来（用于输入记录留存 prompt）
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && path === "/api/foundation") {
      ...
      try {
        // 建书时把实际发给 DeepSeek 的 prompt 留存到输入记录（按 agent 分条）+ 实时进度反馈
        let reqSeq = 0;
    ...
    if (req.method === "GET" && path === "/api/plan") {
      ...
      try {
        ...
        // 提取引擎写入的章纲备忘块（章纲正文），没有则返回全文
        const m = raw.match(/<!--\s*INKOS_PLAN_MEMO_START\s*-->([\s\S]*?)<!--\s*INKOS_PLAN_MEMO_END\s*-->/);
        memo = (m ? m[1] : raw).trim();
      } catch { /* 尚未生成 */ }
